...
extension AppDelegatePanelActionsWiredTests {
    /// team-lead 裁決：`AppDelegate+PanelActions.swift` 的 `switch action` 對這三個新 case
    /// 目前是暫時 stub（`break`）——T10 接線前，故意驗一個現在會失敗的屬性（沒有任何 banner
    /// 出現），讓這條「每個 kind 都要有真副作用」的既有 gate 對這三個新 kind 保持誠實地紅，
    /// 而不是被繞過或標成通過（tested≠wired 的守衛正在做它該做的事）。T10 接線後必須把這裡
    /// 換成真的斷言（連上 `CodexInstaller`／`CodexHookStore` 之後的可觀察效果）。
    @MainActor
