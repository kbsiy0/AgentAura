...
| **正典 §3.7 面板內容（r13／T13k 追加）** | 加 2026-09-22 更新區塊：面板**所有會講「接上了沒有」的字串都必須雙 agent 感知**——三處狀態字串共用 `PanelModel.statusLabel`（D-y，Codex 子句在前、Codex 不在場時逐位元組不變、只有 `.connected` 算已接上）、banner 退場條件依 kind 分流（D-v）、空列句依 Codex 狀態分兩句（D-z）。**並記下這三處在 diff 裡原本一個字都沒被動到**——新增訊號源時要問的是「哪些既有推導因此變成謊」，那正是機器關卡全綠而 persona 判 S0 的原因。**A7 的退場條件沒有 spec 落點**（它只活在 `PanelModel+ConnectCTA.swift` 的 doc comment 裡），所以 D-v 的取代關係寫在這一格，不另外去別的 change 的 spec 裡發明章節 |
...
| **20** | **`optionsExpanded == true` ＋ snippet 卡片同時存在時的總高度不在 CX56 的天花板之內**（§4.10）。Options 展開在 Claude-only 的既有面板就已經 599pt（2026-09-15 實測），是本 change 之前就存在的條件。**T13i 實測（2026-09-21）**：修前 **1849pt**（rows 空）／1840pt（rows=3 對照）；**修後 1056pt**（rows 空）／1047pt（rows=3）——snippet 上限讓它降了約 793pt，**但仍遠超 780pt**，所以這一格維持在 CX56 的域之外、維持明寫接受。修後仍超標的部分主要來自 Options 展開本身（Claude-only 展開就已經 599pt），不是 Codex 卡片 | 覆蓋缺口，明寫（**數字已填**，T13i 2026-09-21） |
