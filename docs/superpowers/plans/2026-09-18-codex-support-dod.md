...
| CX6 | `CodexHooksJSONTests` | ① 寫死 11 個 ② 拿掉 `matcher` ③ 加 `async: true` ④ 漏 `--agent codex` ⑤ **`timeout` 改回 5** ⑥ **`agentFlag` 改成 `"--agent codexx"`**（T04 review M1：字面釘死之前這個 mutation **全量零新紅**） | `everyEntryMatchesF14Verbatim`（12 個 entry 逐格 ＋ 對抗式路徑 round-trip ＋ 跨 module `generatedFlagIsUnderstoodByTheParser`；**T12b 更正**：帳本原記「codexHooksJSONMatchesF14Verbatim」不存在） | | T04 |
...
| CX17 | `CodexInstallerTests` | ① 拿掉內容比對 ② 拿掉 `(dev,ino)` 複查 | `disconnectRemovesOnExactMatch`（＋`disconnectRefusesOnContentMismatch`／`unlinkGuardRefusesWhenIdentityDoesNotMatchCurrentFile`／`unlinkGuardRemovesWhenIdentityMatchesCurrentFile` 共同覆蓋這個不變式；**T12b 更正**：帳本原記「codexDisconnectOnlyRemovesOurBytes」不存在） | | T06 |
| CX18 | `CodexPathScopeTests` | 快照改用字面路徑 | `codexHomeSymlinkWritesInsideResolvedPath` | | T06 |
| CX19 | `CodexStateTests` | ① `symlink` 併進 `.notConnected` ② 判定表第 2／3 列對調 | 七列分別是 `notDirectoryAlwaysUnavailable`／`regularFileMatchingBothIsConnected`／`regularFileMatchingRecordedButNotCurrentIsStale`／`regularFileMismatchIsOccupied`／`nonRegularOccupyingTypesAreOccupied`／`absentWithRejectionIsBlocked`／`absentWithoutRejectionIsNotConnected`，另 `samplesKindsCoverAllSixStates` 守六態聯集（**T12b 更正**：帳本原記「codexStateCoversEveryObservationShape」不存在） | | T07 |
| CX20 | `CodexOptionsRowTests` | ① `.unavailable` 也給「接上 Codex」 ② **被拒時仍給「重新接上」** | `zeroRowStates`／`notConnectedHasOneConnectRow`／`connectedHasOneDisconnectRow`／`stalePathRowsDependOnPathRejection`（四個 `@Test` 皆標「(CX20)」；**T12b 更正**：帳本原記「codexRowsAppearOnlyWhenAvailable」不存在） | | T07 |
| CX21 | `OptionsMenuModelTests`／`CodexOptionsRowTests` | `.connectCodex` 塞進 `nonMenuKinds` | 既有 `rowsUnionCoversEveryMenuKindWithoutDuplicates` ＋ `nonMenuKindsIsExactlyThatLiteralSet`（**T12b 更正**：「optionsRowsCoverEveryAction」是檔案 doc comment／`@Suite` 顯示名用的概念性標籤，不是字面函式名） | | T07 |
| CX22 | `PanelViewModelTests` ＋ `CodexRowLabelPixelTests` | `agentLabel` 對 `.claude` 也給值 | `agentLabelSurvivesThroughPanelModelMake`（model 半，`CodexRowLabelTests.swift`）／`codexLabelProducesPixelDifference`（像素半，`CodexRowLabelRenderTests.swift`；**T12b 更正**：帳本原記「agentLabelOnlyForNonClaude」不存在，「CodexRowLabelPixelTests」這個檔名也不存在） | | T08／T09 |
...
| CX27 | script gate | ① 拿掉腳本第 7 項 ② **拿掉 `--only` 的值域檢查（`[1-7]`）** | `detectsOurCodexHook`（**判準是 `--only 7` 那一行**）；**外加**：`onlyRunsItemSeven`、`onlyWithMissingValueExitsBoundedAndNonZero`（`--only` 缺值，有界時間內非零退出）、`onlyWithInvalidValueExitsNonZeroWithoutClaimingPass`（`--only 77` 非零退出且輸出不含「PASS」）——四支皆在 `VerifyUninstallScriptCodexTests.swift`（**T12b 更正**：帳本原記「verifyUninstallScriptDetectsOurCodexHooks」不存在；本 gate 驗的是 bash 腳本行為，但這四支才是真的 swift test，不是腳本本身） | | T11 |
| CX28 | `HelpDocOptionsRowCoverageTests` | **只刪掉其中一個 Codex 列標題** | `scanCatchesRealCodexOmission`（mutation②正向對照，`@Test(arguments:)` 對 `CodexStateKind.allCases` 取聯集後的兩語言各一條；**T12b 更正**：`allRows` 是產生代表列的 static helper，不是測試本身——CX58 目前抓不到這種「helper 剛好也叫 func」的假陰性，見 known gap） | | T11 |
| CX29 | 文件掃描 | 從 **`SECURITY.md`／`README.md`／`README.zh-TW.md` 任一份**刪掉 `.codex/hooks.json`（**三份各試一次**） | `docListsCodexHooksPath`（`@Test(arguments:)` 參數化三份文件，`SecurityDocCodexPathTests.swift`；**T12b 更正**：帳本原記「securityDocListsEveryPathWeWrite」不存在） | | T11 |
...
| **CX32** | `CodexInstallerClobberTests` | 拿掉 `connect` 第一行的 guard | **帳本原記「codexConnectRefusesBlockedBundlePath」是 MARK 註解分組名，不是函式名——實際函式是 `codexConnectRefusesTranslocated`／`codexConnectRefusesInDownloads`**（且整棵樹零差異；**T12b 重申**：CX58 掃到這一列時，backtick 包住的舊錯名會被誤判成候選——本行已把舊名字改成一般引號，不再誤觸） | **0.007s**（T12 現場重跑，兩條測試都紅） | T06 · **抽驗必做 ✓現場** |
| **CX33** | `CodexHookPathCheckTests` | ① 一律回 `.unsupportedCharacter(" ")` ② 優先序對調 | `returnsFirstOffendingCharacterLeftToRight`（定義域從 `unsupportedCharacters` 推導；**T12b 更正**：帳本原記「codexPathCheckNamesTheOffendingCharacter」不存在） | | T04 |
| **CX34** | `CodexStateTests` | `from` 忽略 `currentExpectedContents` | `regularFileMatchingRecordedButNotCurrentIsStale`（`@Test` 描述裡明寫「(CX34)」；**T12b 更正**：帳本原記「codexStalePathIsDetectedAndOffersReconnect」不存在） | | T07 |
...
| **CX36** | `CodexSectionViewTests` | ① 錯誤文案不插字元（籠統句） ② **被拒時仍畫「重新接上」按鈕** ③ **把 `.mustMoveToApplications` 分支改成會畫 snippet → 必須紅**（T09 review M1：輸入改成真 snippet 之前，這個 mutation 紅 0 條） ④ **T13：把 `.unsupportedCharacter` 分支改回會畫 snippet → 必須紅** ⑤ **T13：拿掉 `.unsupportedCharacter` 的出路句 → 必須紅** ⑥ **T13：`.connectedStalePath` 兩個 rejection 改回共用一句 → 必須紅**（r12 是同一句，證據圖 `10`／`11` 因此位元組完全相同） | 拆成 9 支：`emptyStatesRenderNothing`／`notConnectedShowsPromptAndConnectButton`／`staleWithoutRejectionShowsReconnectButton`／`staleWithRejectionWithholdsReconnectButton`／`staleWithRejectionRendersDifferentTextPerRejectionKind`（⑥）／`occupiedWithSnippetShowsSnippetAndCopyButton`／`occupiedWithoutSnippetWithholdsCopyButton`／`blockedMustMoveShowsExplanationWithoutSnippet`（③）／`blockedUnsupportedCharacterNamesTheCharacterAndOffersAWayOut`（①④⑤，六態 ＋ 兩種 Rejection；**兩個** blocked 那格都**必須餵真的 codexSnippet**；**T12b 更正**：帳本原記「codexSectionRendersEveryState」不存在，那是 `@Suite` 顯示名） | | T09 · **T13d／T13e 擴充** |
...
| **CX41** | `JargonCodexModelTests` | ① Codex 分支回傳 raw（**既有 `everyFixtureModelIsMapped` 也必須紅**，兩條都記） ② 把 `o3` 改成 `O3` ③ **把 Codex 分支移到既有演算法之後 → `gpt-5` 那列必須紅** | 九列分別是 `modelCodexGpt55`／`modelCodexGpt55Codex`／`modelCodexGpt4o`／`modelCodexGpt5`／`modelCodexGpt41Mini`／`modelCodexGpt55High`／`modelCodexO3`／`modelCodexO4Mini`／`modelCodexO3WithBracketSuffix`（既有九列輸出完全不變；**T12b 更正**：帳本原記「jargonModelCoversCodexNaming」不存在，「JargonCodexModelTests」這個檔名也不存在——九支都在 `JargonTests.swift`，不是獨立檔） | | T05 |
...
| **CX46** | `OptionsMenuModel` 呼叫點來源掃描 | 把 view 那一行改回 `codex: .unavailable, codexPathRejection: nil` | `productionCallSitePassesModelCodexState`（`Sources/` 不得出現那兩個字面；**T12b 更正**：帳本原記「optionsRowsCallSitePassesRealCodexState」不存在）。**已知假陰性**：跨行寫法掃不到、註解過濾兩個缺口——主守衛是 T10 的**行為斷言**，本條是便宜的第二道 | | T08 |
