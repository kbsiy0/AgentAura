...
| ③ | Options 展開 → 開「開機自動啟動」開關 | 記錄 ad-hoc 簽章 bundle 上 `SMAppService` 的實際行為：成功／`.requiresApproval` 指引系統設定／其他錯誤 | |
...
| ⑥ | **右鍵**點選單列圖示 | 面板打開且 Options **已展開**；左鍵仍是原本行為（開面板、Options 收合） | |
| ⑦ | 面板打開時按 **⌘Q** | App 真的離開（這個字樣在 phase 2 之前是假的）。另確認：**面板關閉後**按 ⌘Q 不應該再被我們攔截（monitor 要在關閉時移除，否則會攔到全 app 的鍵盤事件） | |
| ⑧ | Options → **減少動態**。系統「減少動態」關閉時切開 → 看燈；再去系統設定打開「減少動態」→ 回來看那一列 | 使用者自開 → 動畫停止；系統強制時該列**鎖住並顯示「系統設定已開啟」**（不得出現「開關說關、動畫卻不動」） | |
| ⑨ | Options → **回報問題…** 與 **關於 AgentAura** | 前者開 GitHub issues；後者顯示版本＋可點的專案連結＋「授權：尚未決定」 | |
| ⑩ | Options 展開後**滑鼠不要移動、直接再點一下** | 不得誤觸任何會改變狀態的控制項（A4 的修法是把第一列換成非破壞性的「說明與快速上手…」） | |
...
| # | 操作 | 預期 | 實際（請填） |
|---|---|---|---|
| ⑪ | 開面板 → 點任一圖例色點開色板 | 色板出現在面板**旁邊**、不與面板重疊（存檔位置若擋住面板就重新擺位；不擋住則尊重使用者搬過的位置） | |
| ⑫ | 改色期間**拖曳色板**、點色板內部 | 面板**不**收起 | |
| ⑬ | 改色期間點桌面／其他視窗 | 面板**收起**，不必回頭點選單列圖示；另確認點選單列圖示本身不會「閃一下又彈開」 | |
