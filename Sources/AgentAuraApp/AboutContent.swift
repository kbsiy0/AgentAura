...
enum AboutContent {
    ...
    /// T29（i18n）：文案搬進 `L10nAbout.license`——沒有預設值，忘了傳要是編譯錯。
    static func license(_ language: Language) -> String { L10nAbout.license.text(language) }

    /// `.credits` 是 standard about panel 唯一能放自由格式文字（含可點的網址）的欄位——
    /// 版本另走 `.applicationVersion`，兩者是 gate 斷言的兩個獨立來源。
    static func options(version: String, language: Language) -> [NSApplication.AboutPanelOptionKey: Any] {
        let credits = NSMutableAttributedString(string: license(language) + "\n\n")
